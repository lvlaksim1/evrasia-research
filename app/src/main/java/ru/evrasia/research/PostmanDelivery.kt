...
internal object PostmanDelivery {
    fun deliver(activity: Activity, json: String, sourceUrl: String = "", environmentJson: String = "") {
        ...
        ResultDelivery.deliverGeneratedFile(activity, "POSTMAN PACKAGE", "postman-$host-$stamp.zip", "application/zip") { output ->
            ZipOutputStream(output).use { zip ->
                ...
                zip.putNextEntry(ZipEntry("postman-$host-$stamp.postman_environment.json")); zip.write(environmentJson.toByteArray(Charsets.UTF_8)); zip.closeEntry()
            }
        }
    }
    ...
    private fun semanticLoginLines(item: JSONObject): List<String> {
        ...
        if (marker != null) {
            ...
            lines.add("  pm.expect(JSON.stringify(payload)).to.include(${JSONObject.quote(marker)});")
        } else {
            lines.add("  if (payload !== undefined) {")
            lines.add("    const text = JSON.stringify(payload).toLowerCase();")
            lines.add("    pm.expect(/\\\"(?:status|result|state|outcome)\\\"\\s*:\\s*\\\"(?:error|failed|failure|unauthorized|forbidden|invalid|login_required|not_authenticated)\\\"/.test(text), \"Authentication response contains an explicit failure marker\").to.eql(false);")
            lines.add("  }")
        }
        lines.add("});")
        return lines
    ...
    private fun observedSuccess(item: JSONObject): String? {
        ...
        for (i in 0 until responses.length()) {
            val body = responses.optJSONObject(i)?.optString("body", "")?.trim().orEmpty()
            if (!body.startsWith("{")) continue
            try {
                val obj = JSONObject(body)
                val status = obj.optString("status", "")
                if (status.lowercase(Locale.US) in setOf("success", "ok", "authenticated", "authorized")) return "\"status\":\"$status\""
                for (key in listOf("success", "ok", "authenticated", "authorized", "valid")) if (obj.opt(key) == true) return "\"$key\":true"
            } catch (_: Exception) {}
        }
        return null
    }
    ...
    private fun findVariableProducer(items: JSONArray, variable: String): JSONObject? {
        for (i in 0 until items.length()) { val item = items.optJSONObject(i) ?: continue; if (scriptContains(item, "test", "pm.collectionVariables.set(${JSONObject.quote(variable)}") || scriptContains(item, "test", "pm.collectionVariables.set('$variable'")) return item }
        return null
    }

    private fun findLoginItem(items: JSONArray): JSONObject? {
        for (i in 0 until items.length()) { val item = items.optJSONObject(i) ?: continue; val name = item.optString("name", ""); if (name.contains(" Login · ") || name.startsWith("Login ·") || Regex("^\\d+\\s+Login\\b", RegexOption.IGNORE_CASE).containsMatchIn(name)) return item }
        return null
    }
    ...
    private fun appendScript(item: JSONObject, listen: String, lines: List<String>) {
        ...
        for (i in 0 until events.length()) if (events.optJSONObject(i)?.optString("listen", "") == listen) { target = events.optJSONObject(i); break }
        if (target == null) { target = JSONObject().put("listen", listen).put("script", JSONObject().put("type", "text/javascript").put("exec", JSONArray())); events.put(target) }
    ...
    private fun scriptContains(item: JSONObject, listen: String, needle: String): Boolean {
        ...
        for (e in 0 until events.length()) { val event = events.optJSONObject(e) ?: continue; if (event.optString("listen", "") != listen) continue; val exec = event.optJSONObject("script")?.optJSONArray("exec") ?: continue; for (i in 0 until exec.length()) if (exec.optString(i, "").contains(needle)) return true }
        return false
    }
}
