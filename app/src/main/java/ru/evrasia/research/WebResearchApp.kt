...
class WebResearchApp : Application(), Application.ActivityLifecycleCallbacks {
    ...
    private val ticker=object:Runnable{override fun run(){syncNetworkStore();debuggerRef.get()?.let{styleDebuggerRows(it)};handler.postDelayed(this,250)}}
    ...
    private fun brandBrowser(a:WebResearchV10Activity){val content=a.findViewById<ViewGroup>(android.R.id.content)?:return;val root=content.getChildAt(0) as? LinearLayout?:return;if(root.tag=="compact-cyan-v22")return;root.tag="compact-cyan-v22";if(root.childCount<7)return;root.setBackgroundColor(ink);val navCard=root.getChildAt(1) as? LinearLayout?:return;val bookmarks=root.getChildAt(2) as? LinearLayout?:return;root.getChildAt(0).visibility=View.GONE;root.getChildAt(3).visibility=View.GONE;root.getChildAt(4).visibility=View.GONE;root.getChildAt(6).visibility=View.GONE;bookmarks.visibility=View.GONE
    ...
    override fun onActivityResumed(a:Activity){when(a){is WebResearchV10Activity->{browserRef=WeakReference(a);syncNetworkStore();brandBrowser(a)};is NetworkDebuggerActivity->{debuggerRef=WeakReference(a);syncNetworkStore();brandDebugger(a)}}}
