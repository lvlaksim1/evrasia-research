...
archive_path = root / 'ResearchArchive.kt'
archive = archive_path.read_text()

old_add = '''    @Synchronized fun addRecord(record: JSONObject) {
        records.put(record)
        NetworkDebugStore.add(record)
    }
'''
new_add = '''    @Synchronized fun addRecord(record: JSONObject) {
        NetworkRecordPipeline.appendRawAndDebug(records, record)
    }
'''
if old_add not in archive:
    raise SystemExit('addRecord block not found')
archive = archive.replace(old_add, new_add, 1)

old_inline = '''            NetworkDebugStore.add(JSONObject()
                .put("source", "js-file")
...
'''
new_inline = '''            NetworkRecordPipeline.addDebuggerOnly(JSONObject()
                .put("source", "js-file")
                .put("time", System.currentTimeMillis())
                .put("method", "JS")
                .put("url", url)
                ...
                .put("responseBody", try { bytes.toString(Charsets.UTF_8) } catch (_: Exception) { "[binary]" }))
'''
if old_inline not in archive:
    raise SystemExit('inline debugger block not found')
archive = archive.replace(old_inline, new_inline, 1)
archive = archive.replace('        NetworkDebugStore.clear()\n', '        NetworkRecordPipeline.clearDebugger()\n', 1)
archive_path.write_text(archive)

Path(root / 'NetworkRecordPipeline.kt').write_text('''package ru.evrasia.research
...
Path('REFACTORING.md').write_text('''# Refactoring
...
print('stage14 architecture audit applied')
